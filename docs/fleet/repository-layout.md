...
from a layer whose selector matches everything, and having two mechanisms for
the same thing would mean explaining which one wins.
...
the classification that selectors match against. Adding a machine to the fleet is
this document and nothing else, unless the machine needs configuration that no
existing layer provides.

## Layer

A `Layer` document declares a selector and a precedence. It contains no resources
...
resource has a selector, and no resource reaches a host by accident.
...
on `apiVersion` and `kind`. Paths matched by `spec.exclude` are skipped.

A document with an unrecognised `kind` is an error rather than something ignored,
because silently skipping a misspelled `kind` would mean a resource quietly not
